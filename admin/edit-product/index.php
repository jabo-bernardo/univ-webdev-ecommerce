        ...
        <div>
            ...
            <div class="p-4">
                <form id="add-product-form">
                    ...
                    <div class="mt-4">
                        <button type="submit" class="p-2 px-6 bg-blue-700 hover:bg-blue-900 text-white font-semibold rounded-md shadow-xl w-full">Save Product</button>
                    </div>
                </form>
            </div>
        </div>
...
<script defer>
    ...
    const handleProductUpload = async (evt) => {
        ...
        const apiResponse = await fetch(`/api/products/update/`, {
            method: "POST",
            body: formData
        });
        const data = await apiResponse.json();
    ...
    const handleProductDataLoad = async () => {
        const apiResponse = await fetch(`/api/products/select/?product_id=${productId}`);
        const data = await apiResponse.json();
        if (!data.success) {
            alert(data.reason);
            return;
        ...
        productImages = product.images.split(",");
        handleProductImagesChange();
    }

    const handlePageLoad = () => {
        handleProductDataLoad();
        handleCategoryLoad();
    }
